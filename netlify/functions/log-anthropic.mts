...
// The Newsletter Studio calls Claude from the browser with the user's own
// API key, so the Anthropic spend never touches a wrapped Netlify function.
// The browser POSTs the usage block from the response here, and we write a
// single row to function_invocations matching the metadata.anthropic shape
// the dashboard already understands.
//
// duration_ms is intentionally 0 so this row contributes no Netlify compute
// credits. We pass timer=null to logInvocation to make that explicit.

const PROJECT = '30-newsletter-studio-goto11';
const FUNCTION_NAME = 'newsletter-studio';
